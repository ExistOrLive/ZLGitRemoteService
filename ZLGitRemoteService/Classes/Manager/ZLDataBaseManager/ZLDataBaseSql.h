...
#define GithubRepositoryTableCreate @"CREATE TABLE GithubRepository (node_id TEXT NOT NULL, repo_id TEXT NOT NULL, full_name TEXT NOT NULL, name TEXT NOT NULL, desc_Repo TEXT, html_url TEXT, isPriva INTEGER, homepage_url TEXT, language TEXT, default_branch TEXT, sourceRepoFullName TEXT, open_issues_count INTEGER, stargazers_count INTEGER, subscribers_count INTEGER, forks_count    INTEGER, updated_at REAL, owner_node_id TEXT, owner_user_id    TEXT, owner_loginName TEXT NOT NULL, owner_html_url TEXT, owner_avatar_url TEXT, owner_type INTEGER, license_node_id TEXT, license_name TEXT, license_spdxId TEXT, license_key TEXT, open_pullRequests_count INTEGER, releases_count INTEGER, discussions_count INTEGER, diskUsage INTEGER, RefreshTime REAL NOT NULL, PRIMARY KEY(full_name))"
...
#pragma mark - update table sql
// V2 添加 open_pullRequests_count releases_count discussions_count diskUsage
#define GithubRepositoryTableUpdate1 @"ALTER TABLE GithubRepository ADD COLUMN open_pullRequests_count INTEGER"

#define GithubRepositoryTableUpdate2 @"ALTER TABLE GithubRepository ADD COLUMN releases_count INTEGER"

#define GithubRepositoryTableUpdate3 @"ALTER TABLE GithubRepository ADD COLUMN discussions_count INTEGER"

#define GithubRepositoryTableUpdate4 @"ALTER TABLE GithubRepository ADD COLUMN diskUsage INTEGER"
...
#define GithubRepoInsert @"insert into GithubRepository (repo_id,node_id,name,full_name,desc_Repo,html_url,isPriva,language,default_branch,sourceRepoFullName,open_issues_count,stargazers_count,subscribers_count,forks_count,updated_at,owner_user_id,owner_node_id,owner_loginName,owner_html_url,owner_avatar_url,owner_type,license_name,license_key,license_node_id,license_spdxId,open_pullRequests_count,releases_count, discussions_count, diskUsage,RefreshTime) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"

#define GithubRepoUpdate @"update GithubRepository set repo_id = ?, node_id = ?, name = ?, desc_Repo = ?, html_url = ?, isPriva = ?, language = ?, default_branch = ?, sourceRepoFullName = ?, open_issues_count = ?, stargazers_count = ?, forks_count = ?, subscribers_count = ?, updated_at = ?, owner_avatar_url = ?, owner_html_url = ?, owner_loginName = ?, owner_node_id = ?, owner_type = ?, owner_user_id = ?,license_key = ?, license_name = ?, license_node_id = ?, license_spdxId = ?, open_pullRequests_count = ?, releases_count = ?, discussions_count = ? , diskUsage = ?,RefreshTime = ? where full_name = ?"
